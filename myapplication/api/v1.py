...
async def read_item(item_id:int, db:Session=Depends(connect_db.get_db)):
    return ItemCrud.read_item(item_id, db)

@router.get("/items", response_model=list[ItemSchema.All])
async def read_items(skip:int=0, limit:int=100, db:Session=Depends(connect_db.get_db)):
...
async def create_item(item:ItemSchema.NeedCreate, db:Session=Depends(connect_db.get_db)):
    return ItemCrud.create_item(item, db)

@router.put("/items/{item_id}", response_model=ItemSchema.All)
async def update_item(item_id:int, new_title:str, new_description:str, db:Session=Depends(connect_db.get_db)):
    if len(new_title) == 0:
...
async def delete_item(item_id:int, db:Session=Depends(connect_db.get_db)):
